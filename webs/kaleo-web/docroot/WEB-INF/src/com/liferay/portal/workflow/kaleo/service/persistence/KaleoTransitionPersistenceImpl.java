...
public class KaleoTransitionPersistenceImpl extends BasePersistenceImpl<KaleoTransition>
	...
	public static final FinderPath FINDER_PATH_FIND_BY_OBC_KALEONODEID = new FinderPath(KaleoTransitionModelImpl.ENTITY_CACHE_ENABLED,
			KaleoTransitionModelImpl.FINDER_CACHE_ENABLED,
			FINDER_CLASS_NAME_LIST, "findByKaleoNodeId",
			new String[] {
				Long.class.getName(),
	...
	public KaleoTransition remove(KaleoTransition kaleoTransition)
		...
		for (ModelListener<KaleoTransition> listener : listeners) {
			listener.onBeforeRemove(kaleoTransition);
		}

		kaleoTransition = removeImpl(kaleoTransition);

		for (ModelListener<KaleoTransition> listener : listeners) {
			listener.onAfterRemove(kaleoTransition);
		}
	...
	public List<KaleoTransition> findByKaleoNodeId(long kaleoNodeId, int start,
		...
		List<KaleoTransition> list = (List<KaleoTransition>)FinderCacheUtil.getResult(FINDER_PATH_FIND_BY_OBC_KALEONODEID,
				finderArgs, this);
		...
		if (list == null) {
			...
			try {
				...
				FinderCacheUtil.putResult(FINDER_PATH_FIND_BY_OBC_KALEONODEID,
					finderArgs, list);
	...
	public KaleoTransition[] findByKaleoNodeId_PrevAndNext(
		...
		try {
			...
			Object[] objArray = QueryUtil.getPrevAndNext(q, count,
					orderByComparator, kaleoTransition);

			KaleoTransition[] array = new KaleoTransitionImpl[3];

			array[0] = (KaleoTransition)objArray[0];
			array[1] = (KaleoTransition)objArray[1];
			array[2] = (KaleoTransition)objArray[2];
		...
		if (listenerClassNames.length > 0) {
			try {
				...
				for (String listenerClassName : listenerClassNames) {
					listenersList.add((ModelListener<KaleoTransition>)Class.forName(
