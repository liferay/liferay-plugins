...
String[] mainSections = ContactsUtil.getPortalPropertiesValue("USERS_FORM_MY_ACCOUNT_MAIN");
String[] identificationSections = ContactsUtil.getPortalPropertiesValue("USERS_FORM_MY_ACCOUNT_IDENTIFICATION");
String[] miscellaneousSections = ContactsUtil.getPortalPropertiesValue("USERS_FORM_MY_ACCOUNT_MISCELLANEOUS");
...
<liferay-util:buffer var="myBuffer">
	...
	<%
	String onSubmit = "event.preventDefault(); " + renderResponse.getNamespace() + "saveForm();";
	%>

	<div id="<portlet:namespace />updateUserDialog">
		<aui:form action="<%= editUserURL %>" method="post" name="fm" onSubmit="<%= onSubmit %>">
	...
	<aui:script>
		Liferay.fire('formNavigator:reveal<portlet:namespace /><%= currSectionId %>');
...
<%
myBuffer = StringUtil.replace(myBuffer, renderResponse.getNamespace(), "_" + PortletKeys.USERS_ADMIN + "_");
%>
