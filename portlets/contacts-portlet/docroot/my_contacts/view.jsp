...
<c:choose>
	<c:when test="<%= group.isUser() %>">
		...
		<%
		List<User> users = UserLocalServiceUtil.getSocialUsers(group.getClassPK(), SocialRelationConstants.TYPE_BI_CONNECTION, 0, 10, new UserLoginDateComparator());
		...
		<c:choose>
			<c:when test="<%= users.isEmpty() %>">
				<div class="portlet-msg-info">
					<liferay-ui:message arguments="<%= HtmlUtil.escape(group.getDescriptiveName(locale)) %>" key="x-has-no-connections" />
				</div>
			</c:when>
			<c:otherwise>
