...
import com.liferay.shortlink.service.util.ShortURLUtil;
...
public class ShortLinkEntryLocalServiceImpl
	 ...
	 */
	@Override
	public ShortLinkEntry addShortLinkEntryWithCheck(ShortLinkEntry link)
		throws ShortURLTakenException, SystemException {

		ShortLinkEntry result;

		if (link.isAutogenerated()) {
			result = storeAutogeneratedShortLinkEntry(link);
		}
		else {
			result = storeCustomShortLinkEntry(link);
		}
	 ...
	 */
	@Override
	public ShortLinkEntry updateShortLinkEntryWithCheck(ShortLinkEntry link)
		throws ShortURLTakenException, SystemException {

		ShortLinkEntry originalLink = fetchShortLinkEntry(
			link.getShortLinkEntryId());

		if (!originalLink.getShortURL().equals(link.getShortURL())) {
			if (isShortURLNotUnique(link.getShortURL())) {
				throw new ShortURLTakenException(link.getShortURL());
			}
		}

		originalLink.setShortURL(link.getShortURL());
		originalLink.setOriginalURL(link.getOriginalURL());
	...
	private ShortLinkEntry createNewShortLinkEntry(ShortLinkEntry link)
		throws SystemException {
		...
		result.setAutogenerated(link.getAutogenerated());
		result.setShortURL(link.getShortURL());
		result.setOriginalURL(link.getOriginalURL());
	...
	private ShortLinkEntry storeAutogeneratedShortLinkEntry(ShortLinkEntry link)
		throws ShortURLTakenException, SystemException {

		List<ShortLinkEntry> links = shortLinkEntryPersistence.findByOURL_A(
			link.getOriginalURL(), true);

		ShortLinkEntry result;

		if (links.isEmpty()) {
			ShortLinkEntry newLink = createNewShortLinkEntry(link);
	...
	private ShortLinkEntry storeCustomShortLinkEntry(ShortLinkEntry link)
		throws ShortURLTakenException, SystemException {

		if (isShortURLNotUnique(link.getShortURL())) {
			throw new ShortURLTakenException(
				"ShortLinkEntry '" + link.getShortURL() + "' is not unique");
		}

		return createNewShortLinkEntry(link);
	}
