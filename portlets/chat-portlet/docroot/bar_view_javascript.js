Liferay.Chat = {
	init: function(params) {
		...
		jQuery('#chat-li > a.item-link').click(
			function() {
			...
			this.ajax,
			{
				...
				success: function(response) {
					...
					if (response.activeBrowser == false) {
						jQuery('ul.portlet-bar .item').removeClass('active');

						Liferay.Chat.prefs.activeBrowser = false;
					}
					else {
						var buddyListItem = jQuery('#chat-li');
						...
						if (response.entries.length) {
							...
							for (i = 0; i < response.entries.length; i++) {
								...
								if (jQuery('ul.portlet-bar .' + chatBuddyCSSname).length == 0) {
									Liferay.Chat.newChat(chatBuddy);
								}

								var o = jQuery('ul.portlet-bar .' + chatBuddyCSSname);
	...
	newChat: function(userId) {
		...
		if (!jQuery('ul.portlet-bar .item').is('.' + userIdClass)) {
			jQuery('ul.portlet-bar').append('<li class="item ' + userIdClass + '"><a class="item-link" href="javascript:;">' + Liferay.Chat.users[userId] + '</a><div class="popup"><div class="chat"><div class="head"><a class="close" href="javascript:;" onClick="Liferay.Chat.chatClose(this);"><span>close</span></a><img class="image" src="' + Liferay.Chat.userIcons[userId] + '">' + Liferay.Chat.users[userId] + '</div><div class="info"></div><div class="show"></div><div class="type"><textarea class="prep"></textarea><textarea class="text" with="' + userId + '"></textarea></div></div></div></li>');
			jQuery('ul.portlet-bar li:last a.item-link').click(
				function() {
					instance.prefs.refreshThrottle = 0;
					instance.prefs.activeBrowser = true;
			...
			var ta = jQuery('ul.portlet-bar li:last .text');
			...
			jQuery('ul.portlet-bar li:last').addClass('waiting');

			instance.chatWindowPopup(jQuery('ul.portlet-bar li:last a.item-link')[0]);
		}
	},
	...
	openChatWindow: function(userId) {
		...
		var newWindow = jQuery('ul.portlet-bar li.' + userIdClass + ' a.item-link');
		var newWindowExists = (newWindow.length > 0);

		instance.newChat(userId);

		if (newWindowExists) {
			instance.chatWindowPopup(jQuery('ul.portlet-bar li.' + userIdClass + ' a.item-link')[0]);
		}
	},
