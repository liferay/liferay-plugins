...
public class TemplatePersistenceImpl extends BasePersistenceImpl<Template>
	...
	/**
	 * Filters the templates before and after the current template in the ordered set where groupId = &#63;.
	 *
	 * <p>
	 * Useful when paginating results. Returns a maximum of <code>end - start</code> instances. <code>start</code> and <code>end</code> are not primary keys, they are indexes in the result set. Thus, <code>0</code> refers to the first result in the set. Setting both <code>start</code> and <code>end</code> to {@link com.liferay.portal.kernel.dao.orm.QueryUtil#ALL_POS} will return the full result set.
	 * </p>
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public Template[] filterFindByGroupId_PrevAndNext(long templateId,
		...
		try {
			...
			array[0] = filterGetByGroupId_PrevAndNext(session, template,
					groupId, orderByComparator, true);

			array[1] = template;

			array[2] = filterGetByGroupId_PrevAndNext(session, template,
	...
	protected Template filterGetByGroupId_PrevAndNext(Session session,
		Template template, long groupId, OrderByComparator orderByComparator,
		boolean previous) {
		StringBundler query = null;
