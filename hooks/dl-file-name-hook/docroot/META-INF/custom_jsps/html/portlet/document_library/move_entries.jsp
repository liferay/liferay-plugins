<%--
/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */
--%>

<%@ include file="/html/portlet/document_library/init.jsp" %>

<%
FileEntry fileEntry2 = (FileEntry)request.getAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRY);

if (fileEntry2 != null) {
	fileEntry2 = _wrap(fileEntry);

	request.setAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRY, fileEntry);
}
else {
	List<FileEntry> fileEntries = (List<FileEntry>)request.getAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRIES);

	for (FileEntry curFileEntry : fileEntries) {
		curFileEntry = _wrap(curFileEntry);
	}

	request.setAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRIES, fileEntries);
}
%>

<liferay-util:include page="/html/portlet/document_library/move_entries.jsp" useCustomPage="<%= false %>" />